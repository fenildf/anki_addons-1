...
def updateNotes( allDb ):
    ...
    for i,( nid, mid, flds, guid, tags ) in enumerate( db.execute( 'select id, mid, flds, guid, tags from notes' ) ):
        ...
        # determine k1e, mx0 and outlier fields
        # populate k1e
        if N_k == 1:
            setField( mid, fs, 'k1e', u'Yes' )
        
        # populate mx0
        if N_m == 0:
            setField( mid, fs, 'mx0', u'Yes' )
            
        # populate outlier
        if N_m >= 1 or N_k >= 2:
            setField( mid, fs, 'outlier', u'Yes' )
        
        # remove outlier
        if N_k <= 1 or N_m == 0:
            setField( mid, fs, 'outlier', u'' )
        
        # determine card type
        if N_m == 0:    # sentence comprehension card, m+0
            ts = ts + [ compTag ]
            setField( mid, fs, jcfg('Field_FocusMorph'), u'' )
            setField( mid, fs, 'mm_FocusMorph_Reading', u'' )
            setField( mid, fs, 'mm_FocusMorph_Audio', u'' )
